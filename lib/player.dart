...
import 'package:audiocutter_example/audiocutter.dart';
...
//      });
  ...
  State<StatefulWidget> createState() {
    return _MediaPlayerState(url, isLocal, mode,_controller,func);
  }
}
...
class _MediaPlayerState extends State<MediaPlayerWidget> {
  ...
  _MediaPlayerState(this.url, this.isLocal, this.mode, this._controller,
      this.func);
  ...
  double _getSliderValue() {
    ...
    return (_position >= _duration ? _duration.inSeconds : _position.inSeconds)
        .toDouble();
  }
        ...
        child: Column(mainAxisSize: MainAxisSize.max,
            children: <Widget>[
              ...
              Row(mainAxisAlignment: MainAxisAlignment.center,
                  ...
                  children: <Widget>[
                    ...
                    IconButton(icon: Icon(Icons.fast_rewind),
                        ...
                        onPressed: () {
                          if (_position < Duration(seconds: 05)) {
                            _seek(_position = Duration(seconds: 0));
                          } else {
                            _seek(
                                _position = _position - Duration(seconds: 05));
                          }
                        }),
                    ...
                    IconButton(
                        icon: _isPlaying
                            ? Icon(
                          Icons.pause_circle_outline, color: Colors.red,)
                            : Icon(
                          Icons.play_circle_outline, color: Colors.teal,),
                        iconSize: 90.0,
                    ...
                    IconButton(icon: Icon(Icons.fast_forward),
                        ...
                        splashColor: Colors.teal,
                        onPressed: () {
                          if (_duration - _position < Duration(seconds: 05)) {
              ...
              Row(mainAxisSize: MainAxisSize.max, children: <Widget>[
                ...
                FlutterSlider(
                    ...
                    tooltip: FlutterSliderTooltip(
                        custom: (value) {
                          return Text(Duration(seconds: value.toInt())
                              .toString()
                              ?.split('.')
                              ?.first ?? '');
                        },
              ...
              Row(mainAxisSize: MainAxisSize.max, children: <Widget>[
                ...
                IconButton(icon: Icon(Icons.content_cut, size: 25,),
                    color: Colors.black,
                    onPressed: () {
                      setState(() {
                        pressed = true;
                ...
                pressed ? SizedBox() : IconButton(
                    ...
                    onPressed:
                      ...
                      var title = file.path
                          .split('/')
                          .last;
                      final ByteData bytes = await ByteData.view(File(url)
                          .readAsBytesSync()
                          .buffer);

                      await Share.file(
                          'Share recoding', title, bytes.buffer.asUint8List(),
                          'audio/*', text: 'My optional text.');
                    }
                ),
                pressed ?
...
//                               initState();
  ...
  Future<String> _cutSong() async {
    ...
    String path = url;
    print("saving audio start " + _upperValue.toString());
    print("saving audio end " + _lowerValue.toString());
  ...
  Widget getSlider() {
//    print("saving audio duration" + _getDurationValue().toString());
//    print("saving audio slider" + getSlider().toString());
    if (pressed) {
      return frs.RangeSlider(
        ...
        onChangeEnd: (double start, double end) {
          _lowerValue = start;
          _upperValue = end;
          _lowerValue2 = start;
          _upperValue2 = end;
         Timer(Duration(milliseconds: 100),(){
          ...
          print(
              'Ended with values: $start and $end');
        },
        onChanged: (start, end) {
          (() {
//            _lowerValue = start;
...
//            _lowerValue2 = start;
//            _upperValue2 = end;
            print("saving audio duration" + start.toString());
    print("saving audio slider" + end.toString());
